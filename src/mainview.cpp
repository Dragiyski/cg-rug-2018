...
MainView::MainView(QWidget *parent) : QOpenGLWidget(parent),
    ...
    planetIllumination{0.3, 0.2, 1.0, 0.0, 1.0},
    sunIllumination{1.0, 0.2, 0.8, 0.0, 1.0},
    horseIllumination{0.3, 0.2, 1.0, 0.6, 50.0},
    universeIllumination{0.8, 0.1, 0.0, 0.0, 1.0},
    universeCenter(0.0, 0.0, 0.0),
    light{QVector3D(0.0, 0.0, 0.0), QVector3D(1.0, 1.0, 0.8)},
...
void MainView::initializeGL()
{
    ...
    {
        std::shared_ptr<Mesh> sun = addMesh(":/models/sphere.obj", ":/textures/sun.png", sunIllumination);
        sun->transform.base.translation = QVector3D(0.0, 0.0, 0.0);
        sun->transform.base.scale = 1.0;
        sun->transform.rotation.state.deltaValue = QVector3D(0.0, 15.0 / 60.0, 0.0);
        sun->name = "Sun";
        namedMeshMap.emplace(std::make_pair(sun->name, sun));
        emit namedMeshAdded(QString::fromStdString(sun->name));
        std::shared_ptr<Mesh> universe = addMesh(":/models/inner-sphere.obj", ":/textures/milky-way.png", universeIllumination);
        universe->name = "Universe";
        universe->transform.base.translation = QVector3D(0.0, 0.0, 0.0);
        universe->transform.base.scale = 80.0;
        std::shared_ptr<Mesh> earth = addMesh(":/models/sphere.obj", ":/textures/earth.png", planetIllumination);
        earth->transform.base.translation = QVector3D(5.0, 0.0, 0.0);
        earth->transform.base.scale = 0.3;
        earth->transform.rotation.state.deltaValue = QVector3D(0.0, 90.0 / 60, 0.0);
        earth->transform.orbit.pivot = QVector3D(0.0, 0.0, 0.0);
        earth->transform.orbit.state.deltaValue = QVector3D(0.0, -10.0 / 60, 0.0);
        earth->name = "Earth";
        namedMeshMap.emplace(std::make_pair(earth->name, earth));
        emit namedMeshAdded(QString::fromStdString(earth->name));
        std::shared_ptr<Mesh> mars = addMesh(":/models/sphere.obj", ":/textures/mars.png", planetIllumination);
        mars->transform.base.translation = QVector3D(8.0, 0.0, 0.0);
        mars->transform.base.scale = 0.3;
        mars->transform.rotation.state.deltaValue = QVector3D(0.0, 60.0 / 60, 0.0);
        mars->transform.orbit.pivot = QVector3D(0.0, 0.0, 0.0);
        mars->transform.orbit.state.deltaValue = QVector3D(0.0, -15.0 / 60, 0.0);
        mars->name = "Mars";
        namedMeshMap.emplace(std::make_pair(mars->name, mars));
        emit namedMeshAdded(QString::fromStdString(mars->name));
        std::shared_ptr<Mesh> horse = addMesh(":/models/horse.obj", ":/textures/horse.png", horseIllumination);
        horse->transform.base.translation = QVector3D(5.0, 1.0, 0.0);
        ...
        horse->transform.orbit.pivot = QVector3D(0.0, 0.0, 0.0);
        horse->transform.orbit.state.deltaValue = QVector3D(0.0, 0.0, 60.0 / 60);
        horse->name = "Horse";
        namedMeshMap.emplace(std::make_pair(horse->name, horse));
        emit namedMeshAdded(QString::fromStdString(horse->name));
        cameraLock = sun;
    }
...
std::shared_ptr<MainView::Mesh> MainView::addMesh(QString objFile, QString textureFile, Illumination illumination)
{
    std::shared_ptr<Mesh> mesh = std::make_shared<Mesh>();
    mesh->illumination = illumination;
    Model model(objFile);

    std::vector<float> meshData = model.getVNTInterleaved_indexed();
    ...
    loadTexture(textureFile, mesh->surfaceTexture);
...
QMatrix4x4 MainView::getMeshTransform(const std::shared_ptr<const MainView::Mesh>& mesh) const
{
    ...
    if(mesh->transform.orbit.pivotObject)
    {
        ...
        QVector3D pivot = getMeshTransform(mesh->transform.orbit.pivotObject).map(QVector3D(0.0, 0.0, 0.0));
...
void MainView::applyUniforms(const std::shared_ptr<Mesh>& mesh)
{
    ...
    shaderActiveProgram->setUniformValue("normalTransform", meshTransform.normalMatrix());
    if(activeMode == ShadingMode::GOURAUD || activeMode == ShadingMode::PHONG)
    {
        if(activeMode == ShadingMode::GOURAUD)
        {
            assert(shaderActiveProgram == &shaderGouraudProgram);
        }
        if (activeMode == ShadingMode::PHONG)
        {
            assert(shaderActiveProgram == &shaderPhongProgram);
        }
        ...
        shaderActiveProgram->setUniformValue("material.emitionFactor", mesh->illumination.emitionFactor);
...
void MainView::updateViewTransform()
{
    ...
    QVector3D viewFront = cameraRotationMatrix.map(modelFront);
    QVector3D viewUp = cameraRotationMatrix.map(modelUp);
    QVector3D viewRight = cameraRotationMatrix.map(modelRight);
    ...
    cameraTransform.setRow(0, QVector4D(viewRight, 0.0));
    cameraTransform.setRow(1, QVector4D(viewUp, 0.0));
    cameraTransform.setRow(2, -QVector4D(viewFront, 0.0));

    QVector3D lockPoint = universeCenter;
    if(cameraLock)
    {
        QMatrix4x4 objectFollowTransform = getMeshTransform(cameraLock);
        lockPoint = objectFollowTransform.map(QVector3D(0, 0, 0));
    }

    QVector3D cameraCenter = cameraTransform.map(-lockPoint);
...
void MainView::setLockObject(QString name)
{
    auto sname = name.toStdString();
    auto pos = namedMeshMap.find(sname);
    if (pos == namedMeshMap.end()) {
        cameraLock.reset();
    } else {
        cameraLock = pos->second;
    }
}
